...
@dataclass
class _HFState(State):
    """
    Streaming state for Hugging Face transformer modules.
    Keeps track of KV cache and batch offsets for incremental decoding.
    """
    ...
    def reset(self):
        """Reset the cached keys/values and offset."""
...
class StreamingHFWrapper(StreamingModule):
    """Wrap a HuggingFace CausalLM so it can participate in the unified streaming system."""

    def __init__(self, hf_model: nn.Module):
        super().__init__()
        self.hf_model = hf_model
        # For safety, ensure HF model is in eval/no-grad mode
        self.hf_model.eval()

    def _init_streaming_state(self, batch_size: int):
        # Similar to other streaming states: lightweight mutable object
        return _HFState(
            past_key_values=None,  # The KV cache
            offset=torch.zeros(1, dtype=torch.long),  # Optional, for logging/debugging
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Equivalent to "state = self._streaming_state"
        state = self._streaming_state

        if state is None:
            # Full-sequence (non-streaming) mode
            outputs = self.hf_model(
                input_ids=input_ids,
                use_cache=False,  # don't store past in training/eval batch mode
                **kwargs,
            )
            return outputs
        ...
        state.offset.add_(input_ids.shape[1])
