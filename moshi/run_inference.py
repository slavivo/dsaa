...
from .models import loaders, MimiModel, LMModel, LMGen, TextLMModel
...
class InferenceState:
    ...
    def __init__(self, model_type: str, mimi: MimiModel, text_tokenizer: sentencepiece.SentencePieceProcessor,
                 lm: LMModel | TextLMModel, batch_size: int, cfg_coef: float, device: str | torch.device, **kwargs):
        self.model_type = model_type
        self.mimi = mimi
        self.text_tokenizer = text_tokenizer
        condition_tensors = get_condition_tensors(model_type, lm, batch_size, cfg_coef)
        self.lm_gen = LMGen(lm, temp=0.0, temp_text=0.0, cfg_coef=cfg_coef, condition_tensors=condition_tensors, **kwargs)
        self.device = device
    ...
    def run(self, in_pcms: torch.Tensor) -> list[tuple[torch.Tensor, torch.Tensor]]:
        ...
        while not all(eos_reached):
            ...
            for b, (one_text, one_pcm) in enumerate(zip(tokens[:, 0].cpu(), out_pcm)):
                if eos_reached[b]:
                    continue
                if not hasattr(self.text_tokenizer, "eos_id"):
                    if i >= 200:
                        eos_reached[b] = True
                ...
                if b == 0:
                    ...
                    if hasattr(self.text_tokenizer, "id_to_piece"):
                    ...
                    # HF tokenizers
                    elif hasattr(self.text_tokenizer, "convert_ids_to_tokens"):
                        token_ids.append(one_text.item())
                        text = self.text_tokenizer.convert_ids_to_tokens(one_text.item())
                        text = text.replace("Ġ", " ").replace("▁", " ").replace("<|endoftext|>", " ")
                        self.printer.print_token(text)
            ntokens += 1
        dt = time.time() - start_time
        self.printer.log("info", f"Token ids: {token_ids}")
...
def main():
    ...
    parser.add_argument("--text-llm", type=str, help="Path of the repo for the text LLM.")
    ...
    if args.text_llm:
        ...
        config = AutoConfig.from_pretrained(args.text_llm, trust_remote_code=True)
    ...
    tokenizer_repo_id = None if not args.text_llm else args.text_llm
