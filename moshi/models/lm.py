...
from ..modules.streaming import StreamingContainer, StreamingModule, State, StreamingHFWrapper
...
class LMModel(StreamingContainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.existing_bos_id = existing_bos_id
...
class TextLMModel(LMModel):
    def __init__(self, *, text_llm: nn.Module, device='cuda', dtype=torch.bfloat16, **lm_kwargs):
        super().__init__(device=device, dtype=dtype, **lm_kwargs)
        ...
        self.text_projector = TextPreservingProjector(text_dim, 4096, 4096, text_dim).to(self.device)
        ...
        self.text_head.to(dtype=text_llm.get_output_embeddings().weight.dtype, device=self.device)

        self.depformer_projectors = nn.ModuleList(
            [ResidualProjector(4096+text_dim+1024, 4096, 1024).to(self.device) for _ in range(self.dep_q)]
        )

        self.depformer_text_emb = MaskedEmbedding(text_llm.get_input_embeddings(), 
                                                  self.zero_token_id)
        original_emb = text_llm.get_input_embeddings()
        embedding_matrix = original_emb.weight.data.float()

        self.depformer_text_proj = nn.Linear(text_llm.config.hidden_size, self.depformer_dim, bias=True).to(self.device)
        ...
        masked_emb = MaskedEmbedding(original_emb, self.zero_token_id)
        text_llm.set_input_embeddings(masked_emb)
        self.text_llm = StreamingHFWrapper(text_llm)

        for p in self.text_llm.parameters():
            p.requires_grad_(False)

    @property
    def text_initial_token_id(self) -> int:
        return self.existing_bos_id if self.existing_bos_id is not None else self.text_card

    def state_dict(self, *args, **kwargs):
            sd = super().state_dict(*args, **kwargs)
            # Drop the text_llm.* keys to keep its weights separate
            return {k: v for k, v in sd.items() if not k.startswith("text_llm.")}
    
    def load_state_dict(self, state_dict, strict=False, assign=False):
        filt = {k: v for k, v in state_dict.items() if not k.startswith("text_llm.")}
        return super().load_state_dict(filt, strict=strict, assign=assign)
    
    def forward_audio(self, audio_codes: torch.Tensor, 
                      sum_condition: torch.Tensor | None = None) -> torch.Tensor:
        B, K, S = audio_codes.shape
        assert K == self.num_codebooks - 1, f"Expected {self.num_codebooks - 1} codebooks, got {K}"
        
        input_ = None # Shape [B, S, dim]
        # Audio embeddings
        for cb_index in range(self.num_audio_codebooks):
            audio_emb = self.emb[cb_index](audio_codes[:, cb_index])
            input_ = audio_emb if input_ is None else input_ + audio_emb
    ...
    def get_depformer_vectors(
        ...
        depformer_vectors = []
        for cb_index in range(Ka):
            # Embedding of last dim K tokens
            if cb_index == 0:
                token_in = self.depformer_text_emb(sequence[:, 0])
                # ALso project if we have text token
                token_in = self.depformer_text_proj(token_in)
            else:
                token_in = self.depformer_emb[cb_index - 1](sequence[:, cb_index + self.audio_offset - 1])
            
            # Concat embedding and temporal vector
            concat_vector = torch.cat([transformer_out, token_in], dim=-1)
            depformer_vectors.append(concat_vector)

        return depformer_vectors
    ...
    def forward_depformer_training(
        ...
        for cb_index in range(Ka):
            ...
            # Concat embedding and temporal vector
            concat_vector = torch.cat([transformer_out, token_in], dim=-1)
            # Process the concat vector via projectors
            if self.depformer_multi_linear:
                linear_index = cb_index
                if self.depformer_weights_per_step_schedule is not None:
                    linear_index = self.depformer_weights_per_step_schedule[cb_index]
                transformer_in = self.depformer_projectors[linear_index](concat_vector)
            else:
                transformer_in = self.depformer_projectors[0](concat_vector)
    ...
    def forward_depformer(
        ...
        if depformer_cb_index == 0:
            ...
            token_in = self.depformer_emb[depformer_cb_index - 1](sequence[:, 0])
        concat_vector = torch.cat([transformer_out, token_in], dim=-1)
        ...
        if self.depformer_multi_linear:
            ...
            if self.depformer_weights_per_step_schedule is not None:
                in_index = self.depformer_weights_per_step_schedule[in_index]
            depformer_input = self.depformer_projectors[in_index](concat_vector)
        else:
            depformer_input = self.depformer_projectors[0](concat_vector)
    ...
    def forward_temporal_audio(
            self, 
            codes: torch.Tensor, 
            sum_condition: torch.Tensor = None
        ) -> tuple[torch.Tensor, torch.Tensor]:
        audio_codes = codes[:, 1:, :]
        audio_out = self.forward_audio(audio_codes, sum_condition)

        return audio_out
    
    def forward_temporal_text(
            self, 
            codes: torch.Tensor,
        ...
        text_out = self.text_llm(text_codes, output_hidden_states=True, output_attentions=True, output_logits=False)
        text_out = text_out.hidden_states[-1]

        temporal_out = torch.cat([text_out, audio_out], dim=-1)

        text_in_head = self.text_projector(temporal_out)
        text_logits = self.text_head(text_in_head).unsqueeze(1)

        return temporal_out, text_logits
    ...
    def get_vectors(
        ...
        audio_out = self.forward_temporal_audio(delayed_codes[:, :, :-1], sum_condition)
        temporal_out, _ = self.forward_temporal_text(delayed_codes[:, :, :-1], audio_out)

        assert temporal_out.shape[0] == delayed_codes.shape[0]
        assert temporal_out.shape[1] == delayed_codes.shape[2] - 1

        depformer_vectors = self.get_depformer_vectors(delayed_codes[:, :, 1:], temporal_out)

        return [temporal_out] + depformer_vectors
    ...
    def forward(
        ...
        audio_out = self.forward_temporal_audio(delayed_codes[:, :, :-1], sum_condition)
        temporal_out, text_logits = self.forward_temporal_text(delayed_codes[:, :, :-1], audio_out)

        assert temporal_out.shape[0] == delayed_codes.shape[0]
        assert temporal_out.shape[1] == delayed_codes.shape[2] - 1

        logits = self.forward_depformer_training(delayed_codes[:, :, 1:], temporal_out)
...
class LMGen(StreamingModule[_LMGenState]):
    def __init__(
        ...
        self.debug_audio_tokens = []
        self.debug_text_tokens = []
        self.debug_audio_logits = []
        self.debug_text_logits = []
    ...
    def _init_streaming_state(self, batch_size: int) -> _LMGenState:
        ...
        if isinstance(lm_model, TextLMModel):
            graphed_main = CUDAGraphed(lm_model.forward_temporal_audio, disable=disable)
            graphed_depth = CUDAGraphed(self.depformer_step, disable=disable)
        elif isinstance(lm_model, LMModel):
    ...
    def step(self, input_tokens: torch.Tensor) -> torch.Tensor | None:
        ...
        if isinstance(lm_model, TextLMModel):
            audio_out = state.graphed_main(input_, state.condition_sum)
            transformer_out, text_logits = lm_model.forward_temporal_text(input_, audio_out)
        elif isinstance(lm_model, LMModel):
        ...
        self.debug_text_logits.append(text_logits) 
        self.debug_text_tokens.append(text_token)
    ...
    def depformer_step(
        ...
        self.debug_audio_logits.append(torch.stack(step_logits, dim=1))  # shape [B, dep_q, 1, card]
        assert len(depformer_tokens) == lm_model.dep_q, (
            len(depformer_tokens),
            lm_model.dep_q,
        )
        out = torch.stack(depformer_tokens, dim=1)
        self.debug_audio_tokens.append(out)
